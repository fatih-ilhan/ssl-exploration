...
class S3VM(BaseEstimator, ClassifierMixin):
    ...
    # ONE vs ALL implementation using binary S3VM classifiers
    ...
    def fit(self, x, y):
        ...
        print('No Classes: ' + str(1 + np.max(y)))
    ...
    def fitParams(self, x_std, y, C, kernel, sigma):
        ...
        self.noClasses = np.max(y) + 1

        yLs = []
        for j in range(self.noClasses):
            y_one = np.zeros((len(yL),), dtype=int)
    ...
    def getClasses(self, xstd):
        ...
        yVals = np.zeros((N, self.noClasses))

        for i in range(N):
            for j in range(self.noClasses):
                y_one = self.ovasvms[j].predictValue(xstd[i])
                yVals[i,j] = y_one

        for i in range(N):
            yhat[i] = np.argmax(yVals[i,:].ravel())
